...
{
    return ((after.tv_sec * NSECS_PER_SEC + after.tv_nsec) - (before.tv_sec * NSEC_PER_MSEC + before.tv_nsec));
}
...
static unsigned long loops_per_msec()
{
    ...
    int ret;
    return NLOOP_FOR_ESTIMATION * NSEC_PER_MSEC / diff_nsec(before, after);
}
...
int main(int argc, char *argv[])
{
    ...
    if (total % resol) {
        fprintf(stderr, "<totl>(%d) should be multiple of <resolution>(%d)\n", total, resol);
        exit(EXIT_FAILURE);
    }
