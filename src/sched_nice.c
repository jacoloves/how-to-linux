...
#define NLOOP_FOR_ESTMATION 1000000000UL
...
static unsigned long loops_per_msec()
{
    ...
    for (i =  0; i < NLOOP_FOR_ESTMATION; i++)
        ;

    clock_gettime(CLOCK_MONOTONIC, &after);

    int ret;
    return NLOOP_FOR_ESTMATION * NSECS_PER_MSEC / diff_nsec(before, after);
}
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3) {
        fprintf(stderr, "usage: %s <total[ms]> <resoultion[ms]>\n", argv[0]);
        exit(EXIT_FAILURE);
    }
